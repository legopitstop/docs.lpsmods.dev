from tkinterplus import Accordion, MaterialIcon
import tkinter
import os
...
test = tkinter.StringVar()  # Track which sections are open. Like a Radiobutton

IMG = MaterialIcon("question_answer", size=(30, 30))

widget = Accordion(root, image=IMG, text="Section 1", name="1", variable=test)
tkinter.Message(widget, text=section1).pack(expand=True)
widget.grid(row=0, column=0, sticky="nesw", padx=20, pady=4)

widget1 = Accordion(root, text="Section 2", name="2", variable=test)
tkinter.Message(widget1, text=section2).pack(expand=True)
widget1.grid(row=1, column=0, sticky="nesw", padx=20, pady=4)

widget2 = Accordion(root, text="Section 3", name="3", variable=test)
tkinter.Message(widget2, text=section3).pack(expand=True)
widget2.grid(row=2, column=0, sticky="nesw", padx=20, pady=4)

widget3 = Accordion(root, text="Section 4", name="4", variable=test)
tkinter.Message(widget3, text=section4).pack(expand=True)
widget3.grid(row=3, column=0, sticky="nesw", padx=20, pady=4)

tkinter.Button(
    root, text="Click Me!", command=lambda: print("Opened", test.get())
).grid(row=4, column=0)
